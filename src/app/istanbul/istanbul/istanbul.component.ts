...
@Component({
  ...
  styleUrls: ['./istanbul.component.scss']
})
export class IstanbulComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute,
    ...
    this.canSkip$ = this.state$.pipe(
      map(state => state.turn && state.actions && state.actions.length > 0));
  }
